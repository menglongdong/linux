...
/* 
 * 二分查找法来进行最大拥塞窗口的查找。该过程发生在拥塞避免阶段，即取发生网络拥塞时的
 * 拥塞窗口作为最大值WMax，上一个好的拥塞窗口最为最小值WMin，在这两个之间使用二分
 * 查找来进行最大窗口的查找。这个过程成为二分查找阶段。
 * 当找到最大窗后，尝试以对称的方式进行拥塞窗口的进一步扩大，这个过程成为窗口探测阶段。
 * 探测阶段也是采用二分的方式来进行的。
 * 
 * 整个过程是通过ACK驱动的，因此调节的单位为RTT。这对于RTT不同的链路，存在一定的不公平
 * 性。对于RTT比较小的场景，比较暴力。
 */
